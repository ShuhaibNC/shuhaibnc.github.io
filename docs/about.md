# ABOUT ME

## TLDR

- If you're looking for general information of mine, Checkout [Wikipedia](https://en.wikipedia.org/wiki/User:ShuhaibNC)
- I have a telegram channel with 17,000 Subscribers, 
Check here [Shuhaib's Channel](https://t.me/shuhaibchannel), 
[ShuhaibsChat](https://t.me/ShuhaibsChat) (Telegram Group) Read [RULES](http://t.me/MissRose_bot?start=rules_-1001666122378) before joining, also read [JustAsk](./notes/justask)
- I have a WhatsApp channel with 2,381 Followers, Check here [WhatsApp Channel](https://whatsapp.com/channel/0029Va9wa0m1Hsq68kqLSQ3r)
- If you're looking for resources that i made? Look here [Internet Database](https://t.me/+liIYPqzMtZA0NDI1)

## Some Telegram Bots i made

- [NightAI Bot](https://t.me/NightAIBot) - A failed attempt of multipurpose experimental bot
- [MovieCrackerBot](https://t.me/MovieCrackerBot) - It can search movie files
- [ScopeGramBot](https://t.me/ScopeGramBot) - A Solution to find Public Private Channels, Groups, Forums with ownership
- [APK info Bot](https://t.me/APKinfoRobot) - Analyzes Android APK files and extracts key technical details instantly. Simply send an APK file
- [Juliet](https://t.me/MissJulietBot) - Telegram group management bot

## Some contributions i made in internet world

#### WABetaInfo

- I'm one of the recognized contributor of [WABetaInfo](https://wabetainfo.com), I assisted in testing and reporting upcoming features in [WhatsApp](https://whatsapp.com/) beta versions for Android.

- https://wabetainfo.com/hall-of-fame

#### xManager Project

- I contributed to the xManager project, an open-source Android tool designed to manage and patch different versions of the [Spotify](https://spotify.com/) application.

- https://github.com/Team-xManager/xManager/graphs/contributors

#### Luckypatcher

- I made custom patches for lucky patcher, [Lucky Patcher](https://www.luckypatchers.com/) is a ‘modifier’ app for unlimited access to android apps, games in play store.

- https://patch.chelpus.com/search?query=shuhaib

#### IMDB Link em all

- I added [MSONE](https://malayalamsubtitles.org) (malayalamsubtitles.org) in it, IMDB-Link-em-all is a customizable user script adding all kinds of links to the IMDb movie info page.

- https://github.com/buzz/imdb-link-em-all/graphs/contributors

#### Fullscreen Toggle Button
- I made a Userscript that add a fullscreen button to bottom-left corner of any mobile view pages, This userscript is designed primarily for mobile phone users who want to view web pages in fullscreen mode. Unlike desktop browsers where users can press the F11 key to enter fullscreen, mobile browsers typically do not provide a direct fullscreen option.

- https://greasyfork.org/en/scripts/542320-fullscreen-toggle-button


## THM 
<iframe src="https://tryhackme.com/api/v2/badges/public-profile?userPublicId=213556" style="border:none; width:329px; height:88px; display:block;"></iframe>